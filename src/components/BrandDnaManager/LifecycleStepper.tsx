...
import { Check, ArrowRight, Sparkles, UserCheck, Shield, Flame, AlertCircle } from 'lucide-react';
...
export const LifecycleStepper: React.FC<LifecycleStepperProps> = ({ currentState, onAdvanceState }) => {
  const currentIndex = STAGES.findIndex((s) => s.state === currentState);

  const handleStageClick = (targetState: LifecycleState, targetIndex: number) => {
  ...
  const handleNext = () => {
    if (currentIndex < STAGES.length - 1) {
      onAdvanceState(STAGES[currentIndex + 1].state);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h3 className="text-xs font-bold text-cyan-300 uppercase tracking-wider font-mono">
            5-Stage Brand DNA Lifecycle Pipeline
          </h3>
        ...
        <div className="flex items-center gap-2">
          ...
          {currentState !== 'USER_REVIEW' && currentState !== 'APPROVED' && currentIndex < STAGES.length - 1 && (
            <button
              onClick={handleNext}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 pt-2">
        {STAGES.map((stage, idx) => {
          ...
          return (
            <div
              key={stage.state}
              onClick={() => handleStageClick(stage.state, idx)}
              className={`${isLastOdd ? 'col-span-2 sm:col-span-1' : ''} p-3.5 rounded-2xl border transition-all cursor-pointer relative overflow-hidden backdrop-blur-xl ${
                isCurrent
